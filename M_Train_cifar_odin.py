...
# Tensorboard
writer = SummaryWriter('cloth_logs/cifar10-odin')
...
def eval_train(model, all_loss):
    ...
    losses = (losses - losses.min()) / (losses.max() - losses.min())  # regularization part
    all_loss.append(losses)  # the loss history is saved into all_loss(think is at as a log file)
    ...
    ##### Since noisy labeled data would have wrong 'TARGET', the CE(outputs, targets) value may follow wierd distribution
